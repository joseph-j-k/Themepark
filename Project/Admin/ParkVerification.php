	...
	if($conn -> query($update))
	{
		...
		else
		{
			...
			echo "<script>
                    ...
                    window.location='ParkRejectList.php';
				  </script>";
	...
	if(isset($_GET["reject"]))
{
	...
	$update = "update tbl_park set park_status = '2' where park_id  = '".$_GET["reject"]."'";
	if($conn -> query($update))
	{
		echo "<script>	
				alert('Verification Unsuccessful');
			    window.location='ParkVerification.php';
			  </script>";
		...
		else
		{
			...
			echo "<script>
					alert('Verification Failed');
                    window.location='ShopParkVerification.php';
				  </script>";
...
<form id="form1" name="form1" method="post" action="">
  <table>
    ...
    <?php
	$i=0; 
	$sel = "select * from tbl_park pa inner join tbl_place p on p.place_id = pa.place_id inner join tbl_district d on d.district_id = p.district_id where park_status = '0' ";
    ...
    <tr>
      ...
      <td>
      <a href="ParkVerification.php?accept=<?php echo $data['park_id'] ?>">Accept</a>
      <a href="ParkVerification.php?reject=<?php echo $data['park_id'] ?>">Reject</a>
      </td>
    </tr>
    <?php } ?>
  </table>
